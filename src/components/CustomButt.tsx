...
export default function CustomButt(props: { label: string, onPress: Function, disable?: boolean, style?: {} }) {
    const [color, setColor] = useState("white"), [opacity, setOpacity] = useState(0.4),
        noTouch = () => {
            setColor("white")
            setOpacity(0.4)
        }
    ...
    return (
        <Pressable disabled={props.disable} style={props.style ? props.style : { flex: 1 }} onTouchStart={() => {
            setColor("green")
            setOpacity(0.8)

        }}
            onTouchCancel={noTouch}
            onTouchEnd={noTouch} onPress={() => props.onPress()}>
